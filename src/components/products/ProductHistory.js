  ...
  const fetchLogs = async () => {
    try {
      ...
      const historyArr = Object.entries(grouped).map(([productId, entry]) => {
        const total = (entry.productSnapshot?.price || 0) * (entry.productSnapshot?.quantity || 0);
        return {
          productId,
      ...
      {historyList.length === 0 ? (
        ...
        <div className="table-responsive d-none d-md-block">
          <div className="d-flex fw-semibold text-muted border-bottom bg-light p-2">
            ...
            <div className="col-1 border-end">Price</div>
            <div className="col-1 border-end">Qty</div>
          ...
          {historyList.map((item) => (
            <div
              key={item.productId}
              className="d-flex align-items-center border rounded p-2 mb-2 history-card"
            >
              <div className="col-2 border-end">{item.name}</div>
              <div className="col-1 border-end">${item.price}</div>
              <div className="col-1 border-end">{item.quantity}</div>
              ...
              <div className="col-2 border-end">${item.total}</div>
              <div className="col-2">
                {item.isDeleted ? (
...
<div className="d-block d-md-none">
  {historyList.map((item) => (
    <div key={item.productId} className="card mb-3 shadow-sm p-2 rounded-4 border-0 bg-light history-card">
      <div className="card-body py-3 px-3">
        ...
        <div className="d-flex justify-content-between mb-2">
          ...
          <span className="fw-bold text-success">${item.total}</span>
        </div>
